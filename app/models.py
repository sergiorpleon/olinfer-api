...
from sqlalchemy import Column, Integer, String, ForeignKey, DateTime, Boolean, func, UniqueConstraint, SmallInteger, Table, Float
...
def from_dict(self, session, **kwargs):
        ...
        if allowed and exists:
            ...
            if is_list:
                ...
                for item in kwargs[rel]:
                    if (
                        ...
                        obj = session.query(cls).filter_by(
                            id=item["id"]).first()
                        col_changes = obj.from_dict(session, **item)
                        if col_changes:
                        ...
                        col = cls()
                        col_changes = col.from_dict(session, **item)
                        query.append(col)
                        session.flush()
                        if col_changes:
                ...
                if self.__mapper__.relationships[rel].query_class is not None:
                    if val is not None:
                        col_changes = val.from_dict(session, **kwargs[rel])
                        if col_changes:
                            changes.update({rel: col_changes})
                else:
                    if val != kwargs[rel]:
...
def to_dict(self, show=None, _hide=[], _path=None):
    ...
    for key in columns:
        ...
        if check in show or key in default:
            ...
            if isinstance(ret_data[key], datetime) == True:
                ret_data[key] = str(ret_data[key])
